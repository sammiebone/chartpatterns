...
void OnTick()
{
    ...
    datetime time[];
    long volume[];
    if(CopyHigh(_Symbol, _Period, 0, LookbackBars, high) < LookbackBars ||
       CopyLow(_Symbol, _Period, 0, LookbackBars, low) < LookbackBars ||
       CopyClose(_Symbol, _Period, 0, LookbackBars, close) < LookbackBars ||
       CopyTime(_Symbol, _Period, 0, LookbackBars, time) < LookbackBars ||
       CopyTickVolume(_Symbol, _Period, 0, LookbackBars, volume) < LookbackBars)
    {
                ...
                if(PositionGetDouble(POSITION_TP) > 0) // TP is set
                {
                    if(PositionGetDouble(POSITION_PROFIT) > 0) // Position is in profit
                    {
                        ...
                        if(StringFind(comment, "TP2") != -1)
                        {
                            ...
                            if(PositionGetDouble(POSITION_SL) < tp1_price)
                                trade.PositionModify(ticket, tp1_price + TrailingStopPlusPips * _Point, PositionGetDouble(
